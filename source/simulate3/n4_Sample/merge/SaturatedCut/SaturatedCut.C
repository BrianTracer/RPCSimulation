...
int main(int argc, char * argv[]) {
  ...
  gRandom->SetSeed(2421257); //233 //245 //189  //257
...
//    cout << "N_t[iStep] is " << N_t[iStep]<< endl;
  ...
  //saturated correct
  int saturated_start = 0;
  int saturated_end = 0;
  for (int iStep = 0;iStep < nSteps ; iStep++){
    if ( N_t[iStep] > 5e7 && N_t[iStep - 1] < 5e7){
      saturated_start = iStep; 
      break;
    }
  }
  ...
  for (int iStep = saturated_start; iStep <= saturated_end && saturated_start >0 && saturated_end >0; iStep++){
    N_t[iStep] = 5e7;
  }
    
  for(int i_result = 0; i_result < nSteps; i_result++){
 //current calculate
    i_t[i_result] = -N_t[i_result] * ElementaryCharge * Ew * vdrift*1e9 *1e3;  //mA
 //voltage calculate
  ...
  for(int i_result = 0; i_result < nSteps; i_result++){
    v_t[i_result] *=exp((-i_result*tStep)/(R*C*1e-3))/(C*1e-12);               //mV
